...
async function doPurge(args: Record<string, unknown>, body: unknown, description: string) {
	const config = resolveConfig(args as Parameters<typeof resolveConfig>[0]);
	const zoneId = resolveZoneId(args as Parameters<typeof resolveZoneId>[0]);

	const { status, headers, data, durationMs } = await request(config, 'POST', purgePath(zoneId), {
		body,
		auth: 'bearer',
		label: `Purging ${description}...`,
	});
	...
	if (status >= 400) {
		...
		console.error('');
		if (status === 429) {
			warn(`Rate limited ${dim(`(HTTP 429, ${formatDuration(durationMs)})`)}`);
		} else {
			console.error(`${symbols.error} ${red('Purge failed')} ${dim(`(HTTP ${status}, ${formatDuration(durationMs)})`)}`);
		}
		for (const e of errors) {
			console.error(`  ${dim(String(e.code ?? status))} ${e.message}`);
		}
		if (d?.denied) {
			console.error(`  ${dim('Denied:')} ${(d.denied as string[]).join(', ')}`);
		}
